...
class MSRN(nn.Module):
    def __init__(self):
        ...
        self.conv_8x = nn.Conv2d(in_channels=64, out_channels=64*8*8, kernel_size=3, stride=1, padding=1, bias=False)
        self.convt_8x = nn.PixelShuffle(8)
    ...
    def forward(self, x):
        ...
        out = self.bottle(out)
        out = self.convt_8x(self.conv_8x(out))
